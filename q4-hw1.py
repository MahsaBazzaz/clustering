...
for i in range(0,n2):
    ...
    #predection of what group each data is belonged to
    index=np.round(out,0)
    #plot each data with color of its group color
    plt.scatter(X1t[i], X2t[i], s=3, c=cdict[index[0]], alpha=1)
    if(error < 0.5):
        correct = correct + 1
